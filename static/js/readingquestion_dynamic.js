; (() => {
    ...
    if (!$)
        return setTimeout(() => {
            updateReadingSubQuestions(updateReadingSubQuestions)
        }, 100)
    ...
    function updateReadingSubQuestions($questionForm) {
        ...
        $questionForm
            .find('.djn-group[data-inline-model="exams-readingsubquestion"] .djn-inline-form:not(.djn-empty-form)')
            .each(function (index) {
                var $form = $(this)
                ...
                if ($title.length === 0) {
                    var titleText =
                        questionType === "mcq_multiple_answer"
                            ...
                            : '<b>Reading sub question:</b>&nbsp;<span class="inline_label">#' + questionNumber + "</span>"

                    $form.prepend(
                        '<h3 class="djn-drag-handler">' +
                        titleText +
                        '<span><a class="inline-deletelink djn-remove-handler djn-level-3 djn-model-exams-readingsubquestion" href="javascript:void(0)">Remove</a></span></h3>',
                    )
                } else {
    ...
    $(document).ready(() => {
        $(document).on(
            ...
            function () {
                ...
                if (
                    $questionForm.find(
                        '.djn-group[data-inline-model="exams-readingsubquestion"] .djn-inline-form:not(.djn-empty-form)',
                    ).length > 0
                ) {
                    updateReadingSubQuestions($questionForm)
                }
            },
        )

        $('.djn-group[data-inline-model="exams-readingquestion"] .djn-inline-form:not(.djn-empty-form)').each(function () {
            updateReadingSubQuestions($(this))
        })
    })
